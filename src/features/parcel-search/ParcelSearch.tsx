...
export default function ParcelSearch({
  ...
  const [resolveStatus, setResolveStatus] = useState<
    "idle" | "resolved" | "ambiguous" | "not_found" | "unavailable"
  >("idle");
  ...
  const cadastralValidation = useMemo(() => {
    const normalized = query.replace(/[:\-\.\s]/g, "");
    if (normalized.length === 0) {
      return { isCadastral: false as const, normalized: "" };
    }
    const result = validateCadastralId(normalized);
    if (result.valid && result.normalized && result.normalized.length === 12) {
      return { isCadastral: true as const, normalized: result.normalized };
    }
    return { isCadastral: false as const, normalized: "" };
  }, [query]);

  const showAutocomplete = useMemo(() => {
    return isFocused && !cadastralValidation.isCadastral && query.trim().length > 0;
  }, [isFocused, cadastralValidation.isCadastral, query]);
  ...
  const handleResolveResult = useCallback(
    (result: ParcelResolveSuccess | ParcelResolveFailure) => {
      if (result.valid) {
        ...
        if (result.error.code === "INVALID_CADASTRAL_ID") {
          setResolveStatus("not_found");
        } else if (
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        if (cadastralValidation.isCadastral) {
          ...
          const addressResult = displayAddressResults[0];
          if (addressResult) {
    ...
    [query, cadastralValidation, displayAddressResults, clearAddressSearch, handleResolveResult, t]
  );
  ...
  const showAddressUnavailable =
    ...
    !isAddressLoading &&
    addressError !== null &&
    addressError.code === "ADDRESS_SEARCH_UNAVAILABLE";
